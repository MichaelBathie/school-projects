...
int iteration = (4096*4)/MAX_WORKERS;
int strt = 0;
...
void compute(int start) {
    ...
    //edit the shared memory variable **DOESN'T WORK**
    mpf_add(*sharedMem, *sharedMem, pi);
    mpf_clear(pi);
    mpf_clear(val1);
    mpf_clear(val2);
    mpf_clear(A);
...
int main(void)
{
    ...
    sharedMem = mmap(NULL, sizeof(mpf_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    mpf_init_set_d(*sharedMem, 0);
    ...
    mpf_mul_ui(*sharedMem, *sharedMem, 12);
    mpf_ui_div (*sharedMem, 1, *sharedMem);

    mpf_out_str(stdout, 10, 0, *sharedMem);

	clock_gettime(CLOCK_REALTIME, &end);
    total = diff(start, end);

    //unmap the shared memory
    munmap(&sharedMem, sizeof(mpf_t));
